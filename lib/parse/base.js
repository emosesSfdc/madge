...
var Base = module.exports = function(src, opts, parent) {
	...
	if (opts.onAddModule) {
        ...
        if (opts.dependencyTransforms){
                this.dependencyTransforms = _.compose.apply(_, opts.dependencyTransforms.reverse());
        } else {
                this.dependencyTransforms = _.identity;
        } 
...
Base.prototype.addModule = function (filename) {
	...
	if (!this.isExcluded(id) && fs.existsSync(filename)) {
                var parsed = this.parseFile(filename);
		this.emit("addModule", {id: id, dependencies: parsed});
                this.tree[id] = parsed.map(this.dependencyTransforms);
	}
};
